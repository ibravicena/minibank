...
$data = mysqli_query($koneksi, "SELECT * FROM tb_nas");
...
if (isset($_POST['go'])) {
  $cari = $_POST['cari'];
  $data_nasabah = mysqli_query($koneksi, "SELECT * FROM tb_nas WHERE nama LIKE '%".$cari."%'");
}else{
  $data_nasabah = mysqli_query($koneksi, "SELECT * FROM tb_nas LIMIT $halaman_awal, $batas");
}
...
<tr>
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  ...
                  <b><p class="modal-title" id="edit-siswa<?= $key['id'] ?>" style="text-align: center; font-size: 18px;">Edit Data Nasabah</p></b>
                </div>
                <div class="modal-body">
  ...
  <div class="form-group">
    ...
    <select class="form-control" name="jns_tab" required="">
       <?php 

  global $koneksi;

  $sql = "SELECT * FROM tb_jtab";
  ...
  foreach ($hasil as $jenis):
   ...
   ?>
      <option value="<?= $jns_tab['jns_tab']?>"><?php echo $jns_tab['jns_tab']; ?></option>
         <?php endforeach; ?>
    </select>
  </div>
  <div class="form-group">
